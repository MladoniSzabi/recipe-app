...
export default function CreateRecipeForm({ onFinished }) {
    ...
    return <div className={style['create-recipe']}>
        <form onSubmit={onFormSubmit}>
            ...
            {errors[2] && <p className={style['error-text']}>*{errors[2].substring(0, errors[1].length - 1)}:</p>}
            <label htmlFor='recipe-img'>Image: </label>
            <p>
                <input onChange={(ev) => validate(ev.target, 1)} type="file" id="recipe-img" name="Image"></input>
                <span>x</span>
            </p>

            {errors[3] && <p className={style['error-text']}>*{errors[3].substring(0, errors[2].length - 1)}:</p>}
            <label htmlFor='recipe-ingredients'>*Ingredients: </label>
            <textarea onChange={(ev) => validate(ev.target, 2)} required name="Ingredient" id="recipe-ingredients" placeholder={'e.g. 200g spaghetti\n2 cans peeled plum tomatoes'}></textarea>

            {errors[4] && <p className={style['error-text']}>*{errors[4].substring(0, errors[3].length - 1)}:</p>}
            <label htmlFor='recipe-method'>*Method: </label>
            <textarea onChange={(ev) => validate(ev.target, 3)} required name="Method" id="recipe-method" placeholder={'e.g. put the pasta in boiling water\nheat a tablespoon of olive oil on medium heat'}></textarea>
